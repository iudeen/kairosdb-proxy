...
pub async fn query_metric_tags_handler(
    ...
    if matches!(state.mode, crate::config::Mode::Simple) {
        ...
        let mut builder = state.client.post(format!("{}{}", url.trim_end_matches('/'), "/api/v1/datapoints/query/tags")).body(body);
    ...
    for (i, metrics_for_backend) in backend_metrics {
        ...
        futs.push(async move {
            ...
            let mut builder = client.post(format!("{}{}", url.trim_end_matches('/'), "/api/v1/datapoints/query/tags")).body(body);
